...
def vit_base_patch32_224(pretrained=False, pretrained_path=None, **kwargs):
    """ ViT-Base with patch size 32.
    """
    model_kwargs = dict(patch_size=32, embed_dim=768, depth=12, num_heads=12, drop_path_rate=0.2, **kwargs)
    model = VisionTransformer(**model_kwargs)
    if pretrained:
        model = load_checkpoint(model, pretrained_path)   
    return model
...
def vit_large_patch16_224(pretrained=False, pretrained_path=None, **kwargs):
    ...
    """
    model_kwargs = dict(patch_size=16, embed_dim=1024, depth=24, num_heads=16, drop_path_rate=0.1, **kwargs)
    model = VisionTransformer(**model_kwargs)
    if pretrained:
        model = load_checkpoint(model, pretrained_path)   
    return model
