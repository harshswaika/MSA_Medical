...
def train_with_validation(model, criterion, optimizers, scheduler, dataloaders, num_epochs, trial, cycle, out_dir, display_every = 5, early_stop_tolerance=100, la=None):
    ...
    for epoch in tqdm(range(num_epochs), leave=False):
        ...
        if (epoch+1)%2==0:
            val_acc, val_f1, spec, sens, cac = testz1(model, dataloaders['val'])
        ...
        if (epoch+1) % display_every == 0:
            train_acc, train_f1, spec1, sens1, cac1 = testz1(model, dataloaders['train'])
            test_acc, test_f1, spec2, sens2, cac2 = testz1(model, dataloaders['test'])
    ...
    train_acc, train_f1, spec1, sens1, cac1 = testz1(model, dataloaders['train'])
    val_acc, val_f1, spec2, sens2, cac2 = testz1(model, dataloaders['val'])
    test_acc, test_f1, spec3, sens3, cac3 = testz1(model, dataloaders['test'])
