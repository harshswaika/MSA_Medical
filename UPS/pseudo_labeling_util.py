...
from .utils_train_test import AverageMeter, accuracy
...
def pseudo_labeling(args, data_loader, model, itr):
    ...
    with torch.no_grad():
        for batch_idx, (inputs, targets, indexs, _) in enumerate(data_loader):
            ...
            interm_nl_mask = ((out_std_nl < args.PL.KAPPA_N) * (out_prob_nl.cpu().numpy() < args.PL.TAU_N)) *1
            ...
            if not args.PL.NO_UNCERTAINTY:
                selected_idx = (max_value>=args.PL.TAU_P) * (max_std.squeeze(1) < args.PL.KAPPA_P)
            else:
                selected_idx = max_value>=args.PL.TAU_P
    ...
    if itr < args.PL.CLASS_BLNC-1:
        min_count = 5000000 #arbitary large value
        for class_idx in range(args.VICTIM.NUM_CLASSES):
    ...
    one_hot_targets = np.eye(args.VICTIM.NUM_CLASSES)[nl_gt_list]
