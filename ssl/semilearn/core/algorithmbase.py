...
class AlgorithmBase:
    ...
    def __init__(
        ...
        **kwargs):
        ...
        self.it = 0
        self.start_epoch = 0
        self.best_eval_acc, self.best_it, self.best_eval_f1, self.best_test_acc = 0.0, 0, 0.0, 0.0
        self.bn_controller = Bn_Controller()
        self.net_builder = net_builder
        self.ema = None

        # additional parameters for model stealing
        ...
        self.optimizer, self.scheduler = self.set_optimizer(self.args.scheduler_type)
    ...
    def set_dataset(self):
        ...
        dataset_dict = get_dataset(self.args, self.algorithm, self.args.dataset, self.args.num_labels, self.args.num_classes, 
                                   self.args.thief_root, self.args.include_lb_to_ulb, self.victim_model, self.labeled_set, self.val_set)
    ...
    def set_data_loader(self):
        ...
        loader_dict['test'] = self.test_loader
    ...
    def set_optimizer(self, scheduler_type='cosine'):
        """
        ...
        elif scheduler_type == 'cosine_fixmatch':
            scheduler = get_cosine_schedule_with_warmup(optimizer,
                                                        self.num_train_iter,
                                                        num_warmup_steps=self.args.num_warmup_iter)
        elif scheduler_type == 'multistep':

            # Multiply the learning rate by a factor of 0.3 at 0.1x, 0.3x and 0.5x of the total training epochs
            steps = [0.1, 0.3, 0.5]
            scale = 3e-1
            steps = sorted([int(step * self.args.epoch) for step in steps])
            print("Learning rate scheduling at steps: ", steps)
            scheduler = torch.optim.lr_scheduler.MultiStepLR(optimizer, steps, scale)
    ...
    def set_model(self):
        ...
        model = self.net_builder(num_classes=self.num_classes, pretrained=False)
        return model
    ...
    def compute_pseudolabels(self, dset='train_ulb', out_key='logits'):
        """
        compute pseudolabels on unlabeled dataset
        """
        self.model.eval()
        # self.ema.apply_shadow()
        ...
        pl_anchor, pl_student = [], []
        trues_anchor, trues_student = [], []
        pl_anchor_unmasked, pl_student_unmasked = [], []
        trues_unmasked = []
        num_labels_anchor, num_labels_student = 0, 0

        ulb_loader = self.loader_dict[dset]
        with torch.no_grad():
            for data in tqdm(ulb_loader):
                # unlabeled data samples
                x = data['x_ulb_w'].cuda()
                
                # victim model's predictions
                logits_victim = self.victim_model(x)
                preds_victim = logits_victim.argmax(axis=1, keepdim=False).detach().cpu()
                ...
                # masked samples
                mask_anchor = self.call_hook('masking', 'MaskingHook', logits_x_ulb=logits_anchor, 
                                             softmax_x_ulb=True).detach().cpu().bool()
                pl_anchor.extend(preds_anchor[mask_anchor].tolist())
                trues_anchor.extend(preds_victim[mask_anchor].tolist())
                ...
                # masked samples
                mask_student = self.call_hook('masking', 'MaskingHook', logits_x_ulb=logits_student, 
                                             softmax_x_ulb=True).detach().cpu().bool()
                pl_student.extend(preds_student[mask_student].tolist())
                trues_student.extend(preds_victim[mask_student].tolist())
                num_labels_student += len(pl_student)
                
                # if len(trues_student)> 1000:
                    # break

        # pseudolabel acc for samples above threshold
        acc_anchor = accuracy_score(trues_anchor, pl_anchor)
        acc_student = accuracy_score(trues_student, pl_student)

        # pseudolabel acc for all samples
        acc_anchor_unmasked = accuracy_score(trues_unmasked, pl_anchor_unmasked)
        acc_student_unmasked = accuracy_score(trues_unmasked, pl_student_unmasked)

        self.model.train()
        pseudolabel_dict = {'anchor_pl_acc': acc_anchor, 
                            'student_pl_acc': acc_student, 
                            'anchor_num_labels': num_labels_anchor,
                            'student_num_labels': num_labels_student,
                            'anchor_pl_unmasked_acc': acc_anchor_unmasked,
                            'student_pl_unmasked_acc': acc_student_unmasked}
...
class ImbAlgorithmBase(AlgorithmBase):
    def __init__(self, args, net_builder, tb_log=None, logger=None, 
                 victim_model=None, labeled_set=None, val_set=None, test_loader=None, **kwargs):
        super().__init__(args, net_builder, tb_log, logger, 
                         victim_model, labeled_set, val_set, test_loader, **kwargs)
