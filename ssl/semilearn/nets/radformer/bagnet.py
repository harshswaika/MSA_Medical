...
class BagNet(nn.Module):
    ...
    def __init__(self, block, layers, strides=[1, 2, 2, 2], kernel3=[0, 0, 0, 0], num_cls=1000, out_channels=2048, avg_pool=True):
        self.inplanes = 64
    ...
    def forward(self, x):
        ...
        x = self.layer2(x)
        x = self.layer3(x)
        attn_features = self.layer4(x)

        if self.avg_pool:
            #after_pool = nn.AvgPool2d(x.size()[2], stride=1)(attn_features)
            after_pool = self.avgpool(attn_features)
...
def BagNet33(pretrain=False, strides=[2, 2, 2, 1], load_local=False, **kwargs):
    ...
    if pretrain:
        if load_local:
            model.load_state_dict(torch.load(model_local_paths["BagNet33"]))
        else:
            model.load_state_dict(model_zoo.load_url(model_urls['BagNet33']), strict=False)
    return model
...
def BagNet17(pretrain=False, strides=[2, 2, 2, 1], load_local=False, **kwargs):
    ...
    if pretrain:
        if load_local:
            model.load_state_dict(torch.load(model_local_paths["BagNet17"]))
        else:
            model.load_state_dict(model_zoo.load_url(model_urls['BagNet17']), strict=False)
    return model
...
def BagNet9(pretrain=False, strides=[2, 2, 2, 1], load_local=False, **kwargs):
    ...
    if pretrained:
        if load_local:
            model.load_state_dict(torch.load(model_local_path["BagNet9"]))
        else:
            model.load_state_dict(model_zoo.load_url(model_urls['BagNet9']), strict=False)
    return model
