...
@ALGORITHMS.register('pseudolabel_with_anchor')
class PseudoLabel_with_anchor(AlgorithmBase):
    """
    ...
    def train_step(self, x_lb, y_lb, x_ulb_w):
        ...
        #                                  )
            ...
            with torch.no_grad():
                logits_anchor_ulb = self.anchor_model(x_ulb_w)['logits']

            # compute mask
            mask = self.call_hook("masking", "MaskingHook", logits_x_ulb=logits_anchor_ulb)


            # generate unlabeled targets using pseudo label hook
            pseudo_label = self.call_hook("gen_ulb_targets", "PseudoLabelingHook", 
                                          logits=logits_anchor_ulb,
                                          use_hard_label=True)
            ...
            unsup_warmup = np.clip(self.epoch / (self.unsup_warm_up * self.epochs),  a_min=0.0, a_max=1.0)
