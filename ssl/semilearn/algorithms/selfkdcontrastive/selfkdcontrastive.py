...
@ALGORITHMS.register('selfkdcontrastive')
class SelfKDContrastive(AlgorithmBase):
    ...
    def info_nce_loss(self, features, n_views=2):
        ...
        labels = torch.cat([torch.arange(self.args.batch_size*self.args.uratio) for i in range(n_views)], dim=0)
        ...
        mask = torch.eye(labels.shape[0], dtype=torch.bool).to(self.args.gpu)
        ...
        labels = torch.zeros(logits.shape[0], dtype=torch.long).to(self.args.gpu)

        logits = logits / self.args.temp_infonce
    ...
    def train_step(self, x_lb, y_lb, x_ulb_w, x_ulb_s):
        ...
        # process multi-vew weak unlabeled data 
        x_ulb_w = torch.cat(x_ulb_w, dim=0)
        ...
        with self.amp_cm():
            ...
            # contrastive loss without pseudolabels
            logits_contr, labels_contr = self.info_nce_loss(feats_x_ulb_w)
            criterion = torch.nn.CrossEntropyLoss().to(self.args.gpu)
            unsup_loss_contr = criterion(logits_contr.cuda(), labels_contr.cuda())
            
            unsup_loss_total = unsup_loss + self.args.alpha_contr * unsup_loss_contr

            total_loss = sup_loss + self.lambda_u * unsup_loss_total
