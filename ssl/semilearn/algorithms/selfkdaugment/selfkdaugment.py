...
@ALGORITHMS.register('selfkdaugment')
class SelfKDAugment(AlgorithmBase):

    """
        SelfKDAugment algorithm (https://arxiv.org/abs/2001.07685).
    ...
    def train_step(self, x_lb, y_lb, x_ulb_w, x_ulb_s):
        ...
        # perform composite data augmentation on labeled samples
        # cutmix
        r = np.random.rand(1)
        if self.args.cutmix_beta > 0 and r < self.args.cutmix_prob:
            # generate mixed sample
            lam = np.random.beta(self.args.cutmix_beta, self.args.cutmix_beta)
            rand_index = torch.randperm(x_lb.size()[0]).cuda()
            target_a = y_lb
            target_b = y_lb[rand_index]
            bbx1, bby1, bbx2, bby2 = rand_bbox(x_lb.size(), lam)
            x_lb[:, :, bbx1:bbx2, bby1:bby2] = x_lb[rand_index, :, bbx1:bbx2, bby1:bby2]
            
            # adjust lambda to exactly match pixel ratio
            lam = 1 - ((bbx2 - bbx1) * (bby2 - bby1) / (x_lb.size()[-1] * x_lb.size()[-2]))

        # mixup
        elif self.args.mixup_alpha > 0:
            # generate mixed sample
            lam = np.random.beta(self.args.mixup_alpha, self.args.mixup_alpha)
            rand_index = torch.randperm(x_lb.size()[0]).cuda()
            target_a = y_lb
            target_b = y_lb[rand_index]
            x_lb = lam * x_lb + (1 - lam) * x_lb[rand_index, :]
        ...
        with self.amp_cm():
            ...
            # cross entropy loss
            if self.args.cutmix_beta > 0 and r < self.args.cutmix_prob:
                loss_ce = self.ce_loss(logits_x_lb, target_a, reduction='mean') * lam + \
                    self.ce_loss(logits_x_lb, target_b, reduction='mean') * (1. - lam)
            elif self.args.mixup_alpha > 0:
                loss_ce = self.ce_loss(logits_x_lb, target_a, reduction='mean') * lam + \
                    self.ce_loss(logits_x_lb, target_b, reduction='mean') * (1. - lam)
            else:
